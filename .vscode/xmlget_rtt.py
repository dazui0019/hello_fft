...
mdk_prj_path = 'MDK-ARM/hello_fft_nucleo.uvprojx'

# 检查是否存在.uvprojx文件
if os.path.isfile(mdk_prj_path) != True:
    print("'hello_fft_nucleo.uvprojx' does not exist.")
    sys.exit(0)
...
if IncludePath is not None:
    ...
    # 删除路径前面的".."
    raw_list = [p.lstrip('../') if p.startswith('../') else (p.lstrip('..\\') if p.startswith('..\\') else p) for p in raw_list]
else:
    raw_list = []
